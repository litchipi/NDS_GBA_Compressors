...
int main(int argc, char **argv) {
  ...
  else if (!strcicmp(argv[1], "-e8")) cmd = CMD_CODE_28;
  else if (!strcicmp(argv[1], "-e4")) cmd = CMD_CODE_24;
#ifdef _CUE_MODES_21_22_
  else if (!strcicmp(argv[1], "-e2")) cmd = CMD_CODE_22;
  else if (!strcicmp(argv[1], "-e1")) cmd = CMD_CODE_21;
#endif
  else if (!strcicmp(argv[1], "-e0")) cmd = CMD_CODE_20;
...
void HUF_CreateCodeTree(void) {
  ...
  for (i = 0; i < num_leafs; i++) {
    ...
    for (j = 0; j < i; j++) {
      ...
      if ((!strcicmp(is, jl) && (tbl[j][0x10] == 'L')) ||
          (!strcicmp(is, jr) && (tbl[j][0x17] == 'L'))) {
        tbl[i][0x06] = 's';
        for (k = 0; k < 6; k++) tbl[i][k+0x10] = '-';
      }

      if ((!strcicmp(is, jl) && (tbl[j][0x11] == 'R')) ||
          (!strcicmp(is, jr) && (tbl[j][0x18] == 'R'))) {
        tbl[i][0x0B] = 's';
